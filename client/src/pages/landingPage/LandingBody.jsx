...
const Hero = ({ theme, subtextClass }) => {
  ...
  return (
    <section className="relative pt-24 pb-12 sm:pt-32 sm:pb-16 lg:pt-36 lg:pb-20 px-4 sm:px-6 overflow-hidden">
      {/* Ambient background glowing orbs */}
      <div
        className="absolute top-10 left-1/4 w-72 sm:w-96 h-72 sm:h-96 bg-indigo-500/15 dark:bg-indigo-500/20 rounded-full blur-3xl pointer-events-none -z-10 animate-pulse-glow"
      />
      <div
        className="absolute top-40 right-4 sm:right-10 w-72 sm:w-96 h-72 sm:h-96 bg-cyan-400/15 dark:bg-cyan-500/15 rounded-full blur-3xl pointer-events-none -z-10 animate-pulse-glow"
      ...
      <div className="container mx-auto grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
        {/* Left Column */}
        <div
          className="space-y-4 sm:space-y-6 text-center lg:text-left animate-in fade-in slide-in-from-bottom-4 duration-500"
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-2.5 sm:gap-3.5 pt-1">
            <Link to="/login" className="w-full sm:w-auto">
              <button className="w-full sm:w-auto flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 sm:px-7 py-3 sm:py-3.5 rounded-2xl font-bold text-xs sm:text-sm shadow-xl shadow-indigo-600/25 hover:shadow-indigo-600/40 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 cursor-pointer">
                Get Started Free
        ...
        {/* Right Column: Responsive Animated Mockup */}
        <div
          className="relative mt-2 lg:mt-0 animate-in zoom-in-95 fade-in duration-500"
        >
          {/* Main Glass Card Preview */}
          <div className="relative rounded-2xl sm:rounded-3xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-xl p-4 sm:p-6 space-y-3 sm:space-y-4 hover:shadow-2xl transition-shadow duration-300">
            {/* Window header */}
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <div className="p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700/60 hover:border-indigo-500/40 transition-colors duration-200">
            ...
            {/* Mock Visual Trend Bars with Interactive Hover Growth */}
              ...
              <div className="flex items-end gap-1.5 sm:gap-2 h-20 sm:h-28 pt-2 px-0.5">
                {[
                  ...
                  <div key={i} className="flex-1 flex flex-col items-center gap-1 h-full justify-end group cursor-pointer">
                    <div
                      className="w-full rounded-t-md sm:rounded-t-lg bg-linear-to-t from-cyan-500 to-indigo-600 opacity-80 group-hover:opacity-100 group-hover:scale-y-105 origin-bottom transition-all duration-300 shadow-xs"
                      style={{ height: bar.h }}
                    />
                    <span className="text-[9px] sm:text-[10px] font-semibold text-slate-400 group-hover:text-indigo-500 transition-colors">{bar.m}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-6">
          {mainFeatures.map((f, i) => (
            <ScrollReveal key={i} animation="animate-fade-in-up" delay={i * 80}>
              <div 
                className={`group p-4 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all duration-300 ${cardClass} shadow-xs hover:shadow-xl hover:border-indigo-500/50 hover:-translate-y-1.5 flex flex-col justify-between cursor-pointer h-full`}
              >
                <div>
                  <div className="mb-3 sm:mb-4 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl inline-block bg-slate-100 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60 group-hover:scale-110 group-hover:border-indigo-500/40 transition-transform duration-300">
                    {f.icon}
                  </div>
                  <h3 className="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-100 mb-1.5 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-200">{f.title}</h3>
                    ...
                    <div className="flex items-end gap-2 sm:gap-3 h-24 sm:h-32 pt-3 pb-1">
                      {[
                        ...
                        <div key={i} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                          <div className="w-full bg-indigo-600 rounded-t-md sm:rounded-t-lg transition-all animate-grow-up" style={{ height: bar.h, animationDelay: `${i * 100}ms` }} />
              ...
              <div className="space-y-2 sm:space-y-3 pt-3">
                {reportTypes.map((type) => (
                  <button
                    ...
                    className={`w-full text-left p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border transition-all duration-200 cursor-pointer ${
                      activeTab === type.id
                        ? 'bg-indigo-600 border-indigo-600 text-white shadow-md shadow-indigo-600/20 scale-[1.01]'
                        : `${cardClass} hover:border-indigo-400 hover:scale-[1.01]`
                    }`}
                  >
    ...
    <section id="stats" ref={sectionRef} className="py-14 sm:py-20 lg:py-28 px-4 sm:px-6">
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-6">
          {stats.map((stat, i) => (
            <ScrollReveal key={stat.label} animation="animate-zoom-in" delay={i * 120}>
              <div 
                className={`rounded-2xl sm:rounded-3xl border p-4 sm:p-7 text-center transition-all duration-300 ${cardClass} shadow-xs hover:shadow-lg hover:-translate-y-1.5 hover:border-indigo-500/40 cursor-pointer h-full flex flex-col justify-center`}
              >
                <div className="text-2xl sm:text-4xl font-black text-indigo-600 dark:text-indigo-400 mb-1 font-mono tracking-tight">
...
const CTA = () => {
  return (
    <section id="cta" className="py-6 sm:py-10 px-3 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <ScrollReveal animation="animate-fade-in-up">
          <div className="bg-linear-to-br from-indigo-600 via-indigo-700 to-indigo-900 rounded-2xl sm:rounded-3xl p-5 sm:py-7 sm:px-6 lg:py-8 lg:px-8 text-center relative overflow-hidden shadow-xl shadow-indigo-600/30 text-white group">
            <div className="relative z-10 max-w-4xl mx-auto space-y-2.5 sm:space-y-3">
              <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-[34px] font-black tracking-tight leading-snug">
                Ready to take control of your expenses?
              </h2>
              <p className="text-indigo-100 text-xs sm:text-sm leading-relaxed max-w-2xl mx-auto">
                Start managing your daily ledgers and generating professional reports in less than two minutes.
              </p>
              <div className="pt-1.5">
                <Link to="/login">
                  <button className="bg-white text-indigo-600 px-6 sm:px-8 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl font-black text-xs sm:text-sm hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer shadow-md hover:shadow-xl">
            ...
            <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-72 sm:w-96 h-72 sm:h-96 bg-white/10 rounded-full blur-3xl pointer-events-none animate-pulse-glow" />
            <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/2 w-72 sm:w-96 h-72 sm:h-96 bg-cyan-400/20 rounded-full blur-3xl pointer-events-none animate-pulse-glow" style={{ animationDelay: '2s' }} />
          </div>
        </ScrollReveal>
      </div>
    </section>
